...
import { signIn } from 'next-auth/react'; // Required for Google Auth
 ...
 try {
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-slate-50 px-4 sm:px-6">
      <div className="w-full max-w-md p-8 sm:p-10 rounded-3xl bg-slate-950 z-10 shadow-2xl">
        
        {/* Header */}
        <div className="text-center mb-8">
          <h1 className="text-2xl font-bold text-red-600 tracking-tight uppercase">
            Join <span className='text-white'>the</span> Club
          </h1>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              required
              value={name}
              className="w-full px-4 py-3 bg-white border border-gray-200 rounded-2xl focus:outline-none focus:ring-1 focus:ring-gray-900 focus:border-gray-900 transition-all text-gray-900 placeholder:text-gray-400"
          ...
          {error && (
            <div className="bg-red-50 text-red-600 text-sm font-medium py-2 px-4 rounded-xl border border-red-100">
              {error}
            </div>
          )}
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full px-4 py-3 mt-2 font-medium text-white bg-red-600 rounded-2xl hover:cursor-pointer hover:bg-red-700 transition-all disabled:bg-gray-300 disabled:cursor-not-allowed shadow-md active:scale-[0.98]"
          >
        ...
        <div className="flex items-center my-6">
          ...
          <span className="shrink-0 mx-4 text-xs font-light text-gray-400 tracking-widest">
            OR
          </span>
          <div className="grow border-t border-gray-800"></div>
        </div>
